...
# %% [markdown]
# #### Black-Scholes American method

option.setPricingEngine(ql.AnalyticAmericanEngine(process))
results.append("Black-Scholes American", option.NPV())
...
# %% [markdown]
# ### Vasicek Analytic method

option.setPicingEngine(ql.AnalyticBlackVasicekEngine(process))
results.append("Vasicek analystic model", option.NPV())
...
# %%
optoption.setPricingEngine(ql.QdPlusAmericanEngine(process))
resresults.append(("QD+", option.NPV()))

# %% [markdown]
# #### Heston semi-analytic formula

# %%
optoption.setPricingEngine(ql.AnalyticHestonEngine(hestonModel))
valvalue = option.NPV()
...
# %%
optoption.setPricingEngine(ql.MCEuropeanEngine(process, "pseudorandom", timeSteps=1, requiredTolerance=0.02, seed=42))
valvalue = option.NPV()

resresults.append(("Monte Carlo (pseudo-random)", value, option.errorEstimate(), abs(value - refValue))
